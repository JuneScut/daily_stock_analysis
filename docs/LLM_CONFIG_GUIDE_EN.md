...
- If the current environment has no valid Agent model path at all, the ask-stock page still returns a failure and surfaces the backend's real configuration diagnosis: `missing_model` means you need `AGENT_LITELLM_MODEL`, `LITELLM_MODEL`, `LLM_CHANNELS`, or legacy provider keys; `agent_mode_disabled` means the explicit `AGENT_MODE=false` kill-switch is still active. Restoring any valid model source, or removing/enabling the explicit switch, recovers the flow without running any migration step.
